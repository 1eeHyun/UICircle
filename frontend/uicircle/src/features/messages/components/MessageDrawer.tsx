...
const MessageDrawer: React.FC<MessageDrawerProps> = ({
  ...
  // Currently selected conversation inside drawer
  ...
  // Close drawer with ESC key and reset selected conversation when closed
  useEffect(() => {
    if (!isOpen) {
  ...
  // Fetch conversations + polling while drawer is open
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Select a conversation in drawer
  const handleSelectConversation = (id: string) => {
    setSelectedConversationId(id);
    onSelectConversation?.(id);
  };

  // Back button in header (from message view → list view)
  const handleBackToList = () => {
    setSelectedConversationId(null);
  };

  // "View all" at the bottom → navigate to /messages page
  const handleViewAll = () => {
    onClose();
    navigate("/messages");
  };
  ...
  return (
    <>
      ...
      >
          ...
          {selectedConversation ? (
            <>
              <button
                ...
                className="mr-2 p-1 rounded-full hover:bg-surface-light transition-colors"
              >
        ...
        <div
          className="border-t border-border-light px-4 py-9 bg-background-light hover:bg-surface-light transition-colors cursor-pointer"
          onClick={handleViewAll}
        >
          <span className="block w-full text-center text-base font-medium text-primary">
            View all messages
          </span>
        </div>
      </div>
